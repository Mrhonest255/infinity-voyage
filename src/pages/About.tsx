...
const About = () => {
  ...
  const heroSubtitle = aboutSettings?.heroSubtitle || 'Your gateway to endless exploration in the heart of East Africa. We craft journeys that transcend the ordinary.';
  ...
  // Stats from database
  const stats = [
    { icon: Users, value: aboutSettings?.stats?.travelers || "5000+", label: "Happy Travelers" },
    { icon: Award, value: aboutSettings?.stats?.experience || "15+", label: "Years Experience" },
    { icon: Globe, value: aboutSettings?.stats?.destinations || "50+", label: "Destinations" },
  ];
        ...
        <div className="relative container-wide mx-auto px-4 md:px-8">
          ...
          >
            ...
            <h1 className="text-5xl md:text-7xl lg:text-8xl font-bold text-white mb-8 leading-tight" style={{ fontFamily: "'Cormorant Garamond', serif" }}>
              {heroTitle.includes(' ') ? (
                <>
                  {heroTitle.split(' ').slice(0, -1).join(' ')} <span className="text-safari-gold italic">{heroTitle.split(' ').slice(-1)}</span>
                </>
              ) : (
                <span className="text-safari-gold italic">{heroTitle}</span>
              )}
            </h1>
        ...
        <div className="container-wide mx-auto px-4 md:px-8">
          <div className="grid lg:grid-cols-2 gap-20 items-center">
            ...
            >
              ...
              >
                <p className="text-6xl font-bold text-safari-night leading-none" style={{ fontFamily: "'Cormorant Garamond', serif" }}>{stats[1].value}</p>

...
const Contact = () => {
  ...
  // Get contact and general settings
  const { settings: contactSettings } = useSiteSettings('contact');
  const { settings: general } = useSiteSettings('general');

  // Dynamic content with fallbacks
  const heroTitle = contactSettings?.heroTitle || "Let's Plan Your Adventure";
  const heroSubtitle = contactSettings?.heroSubtitle || "Have questions? We're here to help create your perfect African experience";
  const officeHours = contactSettings?.officeHours || "Monday - Saturday: 8:00 AM - 6:00 PM (EAT)";
  const responseTime = contactSettings?.responseTime || "24 hours";
  
  // Contact info from general settings
  const companyPhone = general?.phone || '+255 758 241 294';
  const companyEmail = general?.email || 'info@infinityvoyagetours.com';
  const companyAddress = general?.address || 'Kisauni, Stone Town, Zanzibar, Tanzania';
  const WHATSAPP_NUMBER = companyPhone.replace(/\D/g, '');
        ...
        <div className="relative container-wide mx-auto px-4 md:px-8">
          ...
          >
            ...
            <h1 className="text-3xl sm:text-5xl md:text-6xl lg:text-7xl font-semibold text-primary-foreground mb-4 sm:mb-6 leading-tight" style={{ fontFamily: "'Cormorant Garamond', serif" }}>
              {heroTitle.includes(' ') ? (
                <>
                  {heroTitle.split(' ').slice(0, -1).join(' ')} <span className="text-gradient-luxury">{heroTitle.split(' ').slice(-1)}</span>
                </>
              ) : (
                <span className="text-gradient-luxury">{heroTitle}</span>
              )}
            </h1>
              ...
              <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <p className="text-muted-foreground">
                      {companyAddress.split(',').map((line, i) => (
                        <span key={i}>{line.trim()}{i < companyAddress.split(',').length - 1 && <br />}</span>
                      ))}
                    </p>
                  </div>
                </div>
                ...
                <div className="group flex items-start gap-4 p-5 rounded-2xl bg-card shadow-soft border border-border/30 hover:border-safari-gold/30 hover:shadow-elevated transition-all duration-300">
                  ...
                  <div>
                    ...
                    <a href={`tel:${WHATSAPP_NUMBER}`} className="text-muted-foreground hover:text-safari-gold transition-colors block">
                      {companyPhone}
                    </a>
                ...
                <div className="group flex items-start gap-4 p-5 rounded-2xl bg-card shadow-soft border border-border/30 hover:border-safari-gold/30 hover:shadow-elevated transition-all duration-300">
                  ...
                  <div>
                    ...
                    <a href={`mailto:${companyEmail}`} className="text-muted-foreground hover:text-safari-gold transition-colors block">
                      {companyEmail}
                    </a>
                  </div>
                </div>
                ...
                <div className="group flex items-start gap-4 p-5 rounded-2xl bg-card shadow-soft border border-border/30 hover:border-safari-gold/30 hover:shadow-elevated transition-all duration-300">
                  ...
                  <div>
                    ...
                    <p className="text-muted-foreground">
                      {officeHours}<br />
                      <span className="text-safari-gold text-sm font-medium">Response within {responseTime}</span>
                    </p>
                  </div>
                </div>
              </div>
